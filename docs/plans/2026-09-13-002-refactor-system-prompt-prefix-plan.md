...
- R2.（2026-09-13 修订）逐轮变化的常驻数据——时间、记忆清单、知识库命中——不出现在任何发给模型的消息里。会话内不变的**会话常量**（当前只有用户名）放在 system **末尾**：排在最后才不会打断前面可复用的共享前缀；取显示名，平台未给昵称时退回登录名（uid）；OID 与会话号不注入。
...
- 会话常量只收"会话内不变"的值，且必须排在 system 末尾一位；逐轮或跨会话变化的量（时间、记忆清单、知识库命中、技能正文）一律不进 prompt。当前只填用户名（显示名，缺失时退回登录名），会话号保留该槽位待有需要时再放。
...
- **身份可见性**：2026-09-13 已决策并落地——system 末尾注入 `Current user: <显示名>`，显示名为空时退回登录名；OID 不注入。若将来需要 `user_profile` 工具，另立计划。
