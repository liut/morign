...
func ValidHead(rec []string) bool {
	...
	for i, expected := range qaHeads {
		if strings.ToLower(rec[i]) != expected {
			logger().Info("mismatch", "a", rec[i], "b", expected)
			return false
		}
	}
	return true
	...
	for {
		...
		if err != nil {
			if errors.Is(err, io.EOF) {
				logger().Info("import docs", "lines", idx, "valid", valid)
				return nil
			}
			return err
		}
		...
		if len(row) < 3 || len(row[0]) == 0 || len(row[1]) == 0 || len(row[2]) == 0 {
			logger().Info("empty row, skip", "idx", idx, "row", row)
...
func (s *corpuStore) ProcessImportTask(ctx context.Context, task *corpus.ImportTask) error {
	...
	if !ValidHead(rec) {
		logger().Info("invalid import task head", "id", task.StringID())
		return s.finishImportTask(ctx, task, corpus.ImportTaskStatusFailed, 0, 0, 0, 0, nil)
	}
	...
	for {
		...
		if err != nil {
			...
			// CSV 中途解析错误 → 任务失败
			logger().Info("import task csv parse fail", "id", task.StringID(), "err", err)
...
func (s *corpuStore) RecoverImportTasks(ctx context.Context) (int, error) {
	res, err := s.w.db.NewUpdate().
		...
		Exec(ctx)
	if err != nil {
		logger().Info("recover import tasks fail", "err", err)
		return 0, err
	}
...
func (s *corpuStore) importLine(ctx context.Context, basic corpus.DocumentBasic, lw io.Writer) error {
	...
	if err != nil {
		...
		if doc.Content != basic.Content {
			logger().Info("updating", "id", doc.ID, "title", basic.Title, "heading", basic.Heading)
			...
			if lw != nil {
				if _, werr := fmt.Fprintf(lw, "Doc ID: %s, Title: %s, Heading: %s\nDiff:\n%s\n\n",
					...
					logger().Info("write diff fail", "lw", lw, "err", werr)
				}
			}
	...
	if err != nil {
		logger().Info("save document fail", "id", doc.ID, "err", err)
		return err
	}
	return nil
}
...
func (s *corpuStore) afterCreatedCobDocument(ctx context.Context, obj *corpus.Document) error {
	...
	if err != nil {
		logger().Info("create doc vector fail", "dvb", &dvb, "err", err)
		return err
	}
	return nil
}
...
func GetEmbedding(ctx context.Context, text string) (vec corpus.Vector, err error) {
	...
	if err != nil {
		logger().Info("embedding fail", "text", text, "err", err)
		return
	}
	if len(embedding) > 0 {
		...
		logger().Info("embedding res", "text", words.TakeHead(text, 60, ".."), "vec", len(vec))
	} else {
		logger().Info("embedding result is empty", "text", text)
	}
	return
}
...
func (s *corpuStore) MatchDocments(ctx context.Context, ms MatchSpec) (data corpus.Documents, err error) {
	...
	if err != nil || len(ps) == 0 {
		logger().Info("no match docs", "subj", subject, "err", err)
		return
	}
	logger().Info("matched", "docs", ps.Subjects(30), "err", err)
	spec := &CobDocumentSpec{}
	spec.IDs = ps.DocumentIDs()
	err = queryList(ctx, s.w.db, spec, &data).Scan(ctx)
	if err != nil {
		logger().Info("list docs fail", "spec", spec, "err", err)
	} else {
		logger().Info("list docs", "ids", spec.IDs, "matches", data.Headings())
	}
	return
}
...
func matchVectors(ctx context.Context, m vectorMatcher, query string, threshold float32, limit int) (corpus.DocMatches, error) {
	...
	if err != nil {
		logger().Info("GetEmbedding fail", "query", query, "err", err)
		return nil, err
	}
...
func (s *corpuStore) MatchVectorWith(ctx context.Context, vec corpus.Vector, threshold float32, limit int) (data corpus.DocMatches, err error) {
	...
	logger().Debug("match with", "vec", vec[0:5])
	...
	if err != nil {
		logger().Info("match vector fail", "threshold", threshold, "limit", limit, "err", err)
	} else {
		logger().Debug("match vector ok", "threshold", threshold, "limit", limit, "data", data)
	}
	return
}
	...
	for _, doc := range data {
		...
		if err == nil {
			if exist.Subject != subject {
				logger().Info("changed", "sub1", exist.Subject, "sub2", subject)
			}
...
func (s *corpuStore) InvokerForSearch() mcps.Invoker {
	return func(ctx context.Context, args map[string]any) (map[string]any, error) {
		...
		if err != nil || len(subject) == 0 {
			logger().Info("kb search fail: empty subject")
		...
		if len(docs) == 0 {
			logger().Info("matches not found", "subj", subject)
			return mcps.BuildToolSuccessResult("No relevant information found"), nil
		}
		logger().Info("matched", "docs", len(docs))
...
func (s *corpuStore) InvokerForCreate() mcps.Invoker {
	return func(ctx context.Context, args map[string]any) (map[string]any, error) {
		...
		logger().Info("mcp call qa create", "args", args, "user", user)
		...
		if err != nil {
			logger().Info("create document fail", "title", docBasic.Title, "heading", docBasic.Heading,
